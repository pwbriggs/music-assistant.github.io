...
- The Snapcast provider is not enabled by default. An inbuilt Snapserver is launched when this provider is enabled. Snapweb is enabled on this server. Once enabled the workings of the server are transparent and the clients appear in the MA UI
- Music Assistant currently implements Snapcast version 0.27.0 only; future updates may include support for newer versions
- The Snapcast provider will use the inbult Snapserver by default although a switch in the settings allows the use of an external server if desired. When using an external server the server IP and port must be entered. 
...
## Enable SnapWeb on External Server
